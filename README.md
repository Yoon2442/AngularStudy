...
> })
  ...
  > Angular CLI 업데이트 후 기존 로컬 폴더에서 컴파일 진행할 경우 에러가 발생하여 새 프로젝트를 만든 후 `src`폴더를 복사하여 다시 진행했다.
  > 업그레이드한 버전으로 컴파일 진행했을 때 모듈이 없어서 오류가 발생하여 `npm install --save core-js@^2.5.0` 명령어로 모듈을 설치했다.  [[참고 사이트]]  
